<x-front-layout>
    <x-slot name="title">{{ __('comunidade.title') }}</x-slot>
    ...
    <section class="relative h-screen w-full overflow-hidden flex items-start justify-center">
        ...
        <div class="relative z-10 text-center text-white px-6 pt-[120px]">
            <p class="text-lg sm:text-xl font-semibold tracking-[2px] mb-3 opacity-0 translate-y-8" style="animation: heroSlideUp 0.8s ease-out 0.3s forwards;">{{ __('comunidade.hero.eyebrow') }}</p>
            <h1 class="text-2xl sm:text-4xl font-light leading-snug max-w-3xl mx-auto opacity-0 translate-y-8" style="animation: heroSlideUp 0.8s ease-out 0.5s forwards;">{{ __('comunidade.hero.title') }}</h1>
        </div>
    </section>
        ...
        <div class="content-container">
            <div class="max-w-2xl mx-auto">
                <div class="mb-10 md:mb-12 text-center animate-up">
                    <p class="text-lg font-semibold tracking-[2px] text-[#191919] mb-3">{{ __('comunidade.form.eyebrow') }}</p>
                    <h2 class="text-2xl sm:text-4xl font-light text-[#191919] mb-4">{{ __('comunidade.form.heading') }}</h2>
                    <p class="text-base md:text-lg text-gray-500 font-light">{{ __('comunidade.form.intro') }}</p>
                </div>
                ...
                <form action="{{ route('sobre.comunidade.store') }}" method="POST" class="bg-[#F8F9F9] p-8 md:p-10 space-y-5 animate-up rounded-sm text-gray-800">
                    ...
                    <button type="submit" class="w-full bg-black text-white py-3 px-4 rounded hover:bg-gray-900 transition-colors uppercase tracking-widest text-sm font-medium mt-2">{{ __('comunidade.form.submit') }}</button>
                </form>
            </div>
        </div>
    </section>
